...
export const fetchUserDrafts = (): Promise<DraftInfo[]> =>
  apiFetch('/test-explorer/drafts');

export const fetchDraftCount = (): Promise<{ count: number }> =>
  apiFetch('/test-explorer/draft-count');

export const fetchDraftPaths = (): Promise<string[]> =>
  apiFetch('/test-explorer/draft-paths');

export const saveDraftApi = (repoId: string, filePath: string, data: {
  branch: string;
...
export const deleteDraftApi = (repoId: string, filePath: string, branch = 'main'): Promise<{ success: boolean }> =>
  apiFetch(`/test-explorer/draft/${repoId}/${filePath}?branch=${encodeURIComponent(branch)}`, { method: 'DELETE' });

export const submitDraftsApi = (data: { draftIds: string[]; prTitle: string; prDescription?: string }): Promise<{ success: boolean; prUrl: string; prNumber: number; filesCommitted: number }> =>
  apiFetch('/test-explorer/submit-drafts', { method: 'POST', body: JSON.stringify(data) });
