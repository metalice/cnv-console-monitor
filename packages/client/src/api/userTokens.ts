...
export const fetchUserTokens = (): Promise<UserTokenInfo[]> => apiFetch('/user/tokens');

export const saveUserTokenApi = (
  provider: string,
  token: string,
): Promise<{ success: boolean; username?: string; email?: string }> =>
  apiFetch(`/user/tokens/${provider}`, { body: JSON.stringify({ token }), method: 'PUT' });

export const deleteUserTokenApi = (provider: string): Promise<{ success: boolean }> =>
  apiFetch(`/user/tokens/${provider}`, { method: 'DELETE' });

export const testUserTokenApi = (
  provider: string,
): Promise<{ success: boolean; username?: string; email?: string }> =>
