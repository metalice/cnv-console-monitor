...
import { type CreateReportRepo, type ReportRepo } from '@cnv-monitor/shared';
        ...
        <Form isHorizontal>
          ...
          <FormSection title="Scope" titleElement="h3">
            <FormGroup isRequired fieldId="wr-component" label="Component">
              ...
              <HelperText>
                <HelperTextItem>
                  When you filter the team report by component, only repos assigned to that
  ...
  <Modal isOpen={isOpen} variant={ModalVariant.small} onClose={onClose}>
    ...
    <ModalBody>
      <Content component="p">
        ...
        this repository in future team reports.
      </Content>
    </ModalBody>
    <ModalFooter>
...
export const ReportRepoSettings = () => {
  const queryClient = useQueryClient();
  const { availableComponents } = useComponentFilter();

  const { data: repos = [] } = useQuery({
    queryFn: () => fetchReportRepos(),
    queryKey: ['reportRepos'],
    staleTime: FIVE_MINUTES_MS,
  });

  const createMutation = useMutation({
    mutationFn: (data: CreateReportRepo) => createReportRepoApi(data),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['reportRepos'] });
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ data, id }: { data: Partial<CreateReportRepo>; id: string }) =>
      updateReportRepoApi(id, data),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['reportRepos'] });
    },
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteReportRepoApi(id),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['reportRepos'] });
    },
  });
  ...
  const [deleteTarget, setDeleteTarget] = useState<ReportRepo | null>(null);
  ...
  const openEdit = (repo: ReportRepo) => {
    setInitialForm({
  ...
  const handleToggleEnabled = (repo: ReportRepo) => {
        ...
        <CardBody>
          {repos.length === 0 ? (
            <EmptyState headingLevel="h3" icon={RepositoryIcon} titleText="No repositories">
              <EmptyStateBody>
                Add GitHub or GitLab repositories to collect PRs, commits, and MRs for the team team
                report. Each component can have its own set of repos. Authentication uses the global
                tokens from the Integrations section above.
              </EmptyStateBody>
            </EmptyState>
          ) : (
            <Table aria-label="Report repositories" variant="compact">
              <Thead>
                <Tr>
                  <Th>Repository</Th>
                  <Th>Component</Th>
