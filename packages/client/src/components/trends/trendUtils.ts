import type { AIPredictionAccuracy, HeatmapCell, VersionTrendPoint } from '@cnv-monitor/shared';
...
export const buildVersionGroups = (data: VersionTrendPoint[] | undefined): VersionGroups => {
  ...
  const versions = [...new Set(data.map(point => point.version))].toSorted(
    (leftVersion, rightVersion) =>
      leftVersion.localeCompare(rightVersion, undefined, { numeric: true, sensitivity: 'base' }),
  );
  const dates = [...new Set(data.map(point => point.date))].toSorted((leftDate, rightDate) =>
  ...
  return { byVersion, dates, versions };
};

export type HeatmapData = {
  tests: { unique_id: string; name: string; fail_count: number }[];
  dates: string[];
...
export const buildHeatmap = (data: HeatmapCell[] | undefined): HeatmapData | null => {
  ...
  const dates = [...new Set(data.map(cell => cell.date))].toSorted((leftDate, rightDate) =>
    leftDate.localeCompare(rightDate),
  );
  ...
  for (const cell of data) {
    if (!seen.has(cell.unique_id)) {
      ...
      tests.push({ fail_count: cell.fail_count, name: cell.name, unique_id: cell.unique_id });
    }
    cellMap.set(`${cell.unique_id}|${cell.date}`, cell.status);
  }

  return { cellMap, dates, tests };
};
...
export type AIMatrix = {
  ...
  accuracies: { prediction: string; accuracy: number }[];
};
...
export const buildAIMatrix = (data: AIPredictionAccuracy[] | undefined): AIMatrix | null => {
  ...
  for (const prediction of predictions) {
    matrix.set(prediction, new Map());
    totals.set(prediction, 0);
  }
  ...
  const accuracies = predictions.map(prediction => {
    ...
    return { accuracy: Math.round((correct / total) * 100), prediction };
  });
  return { accuracies, actuals, matrix, predictions, totals };
};
...
export const computeVersionHealth = (data: VersionTrendPoint[] | undefined): VersionHealth => {
  ...
  for (const [version, rates] of versionRates) {
    ...
    if (!best || avg > best.rate) {
      best = { rate: Math.round(avg * 10) / 10, version };
    }
    if (!worst || avg < worst.rate) {
      worst = { rate: Math.round(avg * 10) / 10, version };
    }
  }
  return { best, worst };
};
...
export const computeSummaryStats = (
  trends: { total: number; passed: number }[] | undefined,
): SummaryStats => {
