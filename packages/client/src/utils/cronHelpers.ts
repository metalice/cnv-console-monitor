...
export const parseCron = (cron: string): CronParsed => {
  ...
  if (daysPart === '*') {
    return { days: new Set(ALL_DAY_IDS), hour, minute };
  }
  ...
  return { days: new Set(dayIds), hour, minute };
};
...
export const formatScheduleLabel = (cron: string): string => {
  const { days, hour, minute } = parseCron(cron);
  ...
  if (
    days.size === 5 &&
    [...WEEKDAY_IDS].every(dayId => days.has(dayId)) &&
    !days.has('0') &&
    !days.has('6')
  ) {
