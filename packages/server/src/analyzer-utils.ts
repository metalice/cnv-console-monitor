import { LaunchRecord, TestItemRecord, getTestFailureStreak, FailureStreakInfo, RunStatus } from './db/store';
...
export const parseTier = (launch: LaunchRecord): string => {
  if (launch.tier && launch.tier !== '-') {
    ...
    if (tierUpper === 'TIER-0' || tierUpper === 'TIER0') return 'T0';
    if (tierUpper === 'TIER-1' || tierUpper === 'TIER1') return 'T1';
    if (tierUpper === 'TIER-2' || tierUpper === 'TIER2') return 'T2';
    if (tierUpper === 'TIER-3' || tierUpper === 'TIER3') return 'T3';
    if (tierUpper === 'TIER-4' || tierUpper === 'TIER4') return 'T4';
    if (tierUpper === 'UPGRADE') return 'Upgrade';
    return launch.tier;
  }
...
export const computeHealth = (launches: LaunchRecord[]): HealthStatus => {
  if (launches.some(launch => launch.status === 'FAILED')) return 'red';
  if (launches.some(launch => launch.status === 'IN_PROGRESS')) return 'yellow';
  return 'green';
}
