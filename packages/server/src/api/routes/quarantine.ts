...
import { getQuarantines, getQuarantineById, getQuarantineLogs, getQuarantineStats, getQuarantineHistory, updateQuarantineStatus, addQuarantineLog } from '../../db/store';
...
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await getQuarantines({ status, component, limit, offset });
    res.json(result);
  } catch (err) { next(err); }
});
...
router.get('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!q) { res.status(404).json({ error: 'Quarantine not found' }); return; }
...
router.post('/:id/resolve', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await unquarantineTest(req.params.id as string, req.user?.email || 'unknown', parsed.data);
    res.json(result);
  } catch (err) { next(err); }
});
...
router.post('/:id/approve', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const q = await getQuarantineById(req.params.id as string);
    if (!q) { res.status(404).json({ error: 'Quarantine not found' }); return; }
    if (q.status !== 'proposed') { res.status(400).json({ error: 'Only proposed quarantines can be approved' }); return; }
...
router.post('/:id/reject', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const q = await getQuarantineById(req.params.id as string);
    if (!q) { res.status(404).json({ error: 'Quarantine not found' }); return; }
    if (q.status !== 'proposed') { res.status(400).json({ error: 'Only proposed quarantines can be rejected' }); return; }

    await updateQuarantineStatus(q.id, 'resolved', { resolved_by: req.user?.email || 'unknown', resolved_at: new Date() });
    await addQuarantineLog(q.id, 'rejected', req.user?.email || 'unknown', { reason: req.body.reason || '' });
