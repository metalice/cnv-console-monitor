...
import { runParallelPollCycles, runReportPollCycle } from '../../report/poller';
import { getAllPollStatuses, getReportPollStatus, isAnyPollRunning } from '../../report/pollState';

export const reportPollRouter = Router();

reportPollRouter.post('/', (req, res) => {
  ...
  if (components) {
    ...
    const running = componentList.some(comp => {
      const status = getReportPollStatus(comp);
  ...
  if (isAnyPollRunning()) {
    res
      .status(409)
      .json({ error: 'A poll is already running', status: getReportPollStatus(component) });
    return;
  }
...
reportPollRouter.get('/status', (req, res) => {
  ...
  res.json(getReportPollStatus(component));
});
