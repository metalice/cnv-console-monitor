...
import { fetchAllItemsForLaunch, refreshLaunchTestItems } from '../../poller';
import { clampInt, parseIntParam } from '../middleware/validate';
...
router.get('/launch/:launchId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let items =
      status === 'FAILED' ? await getFailedTestItems(launchId) : await getAllTestItems(launchId);
...
router.get('/launches', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (ids.length === 0) {
      res
        .status(400)
        .json({ error: 'ids query parameter required (comma-separated launch rp_ids)' });
      return;
    }
