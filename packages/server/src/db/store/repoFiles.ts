...
export const getFilesByRepo = async (repoId: string, branch = 'main'): Promise<RepoFile[]> => {
  return repoFiles().find({ where: { repo_id: repoId, branch }, order: { file_path: 'ASC' } });
};

export const getFilesByType = async (repoId: string, fileType: string, branch = 'main'): Promise<RepoFile[]> => {
  return repoFiles().find({ where: { repo_id: repoId, branch, file_type: fileType }, order: { file_path: 'ASC' } });
};

export const getFileByPath = async (repoId: string, branch: string, filePath: string): Promise<RepoFile | null> => {
  return repoFiles().findOneBy({ repo_id: repoId, branch, file_path: filePath });
};

export const upsertRepoFile = async (data: Partial<RepoFile>): Promise<RepoFile> => {
  const existing = data.repo_id && data.branch && data.file_path
    ? await repoFiles().findOneBy({ repo_id: data.repo_id, branch: data.branch, file_path: data.file_path })
    : null;
...
export const updateFileCounterpart = async (id: string, counterpartId: string | null): Promise<void> => {
...
export const getRepoFileStats = async (repoId: string, branch = 'main'): Promise<{ docs: number; tests: number; matched: number }> => {
  ...
  `, [repoId, branch]);
  const r = rows[0] || {};
  return { docs: parseInt(r.docs ?? '0', 10), tests: parseInt(r.tests ?? '0', 10), matched: parseInt(r.matched ?? '0', 10) };
};
