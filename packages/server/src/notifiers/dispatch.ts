...
export const filterReportForSubscription = (report: DailyReport, sub: SubscriptionRecord): DailyReport => {
  if (sub.components.length === 0) return report;
  ...
  filtered.untriagedCount = filtered.groups
    ...
    .filter(item => !item.defect_type || item.defect_type === 'ti001' || item.defect_type?.startsWith('ti_'))
    .length;

  return filtered;
};

export const dispatchToSubscription = async (report: DailyReport, sub: SubscriptionRecord): Promise<string[]> => {
  ...
  if (sub.slackWebhook) {
    try {
      ...
      log.info({ subId: sub.id, name: sub.name }, 'Slack sent');
    } catch (err) {
  ...
  if (sub.emailRecipients.length > 0) {
    try {
      ...
      log.info({ subId: sub.id, name: sub.name }, 'Email sent');
    } catch (err) {
