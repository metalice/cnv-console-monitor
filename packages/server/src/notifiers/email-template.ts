...
import { healthColor, emailCss, streakBarHtml, formatLastPass } from './email-styles';
...
const buildComponentSections = (report: DailyReport): string => {
  ...
  if (allPassed.length > 0) html += `<p class="green-list">All green: ${allPassed.join(', ')}</p>`;
  return html;
}
...
const buildFailedSection = (group: LaunchGroup): string => {
  ...
  if (dashboardUrl) html += `<a href="${dashboardUrl}/launch/${group.latestLaunch.rp_id}" style="font-size: 13px;">View in Dashboard →</a>`;
  return html;
}
...
<body>
  ...
  ${report.newFailures.length > 0 ? `<div class="section-title">New Failures (${report.newFailures.length})</div>
