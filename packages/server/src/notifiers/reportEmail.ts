...
import { formatDateRange, pluralize, type TeamReport } from '@cnv-monitor/shared';

import { config } from '../config';
import { logger } from '../logger';

const log = logger.child({ module: 'TeamReport:Email' });

export const buildReportEmailHtml = (report: TeamReport): string => {
...
export const sendReportEmail = async (report: TeamReport, recipients: string[]): Promise<void> => {
  ...
  log.info({ recipients: recipients.length, weekId: report.weekId }, 'Team report email sent');
};
