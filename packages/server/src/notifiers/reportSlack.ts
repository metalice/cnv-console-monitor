...
const log = logger.child({ module: 'TeamReport:Slack' });

const TIMEOUT_MS = 10_000;

export const buildReportSlackBlocks = (report: TeamReport): Record<string, unknown>[] => {
...
export const sendReportSlack = async (report: TeamReport, webhookUrl: string): Promise<void> => {
  const blocks = buildReportSlackBlocks(report);
  await axios.post(webhookUrl, { blocks }, { timeout: TIMEOUT_MS });
  log.info({ weekId: report.weekId }, 'Slack team report sent');
};
