...
const streakBar = (statuses: string[]): string =>
  `[${statuses
    .map(status => (status === 'FAILED' ? 'X' : status === 'PASSED' ? '-' : '?'))
    .join('')}]`;
...
export const buildBlocks = (report: DailyReport): SlackBlock[] => {
  ...
  blocks.push({
    text: { emoji: true, text: `Console Dashboard Report — ${report.date}`, type: 'plain_text' },
    type: 'header',
  });
  ...
  for (const [component, groups] of [...groupsByComponent.entries()].sort((a, b) =>
    ...
    if (failedGroups.length === 0 && greenGroups.length === 0) {
      continue;
    }
  ...
  if (report.newFailures.length > 0) {
    ...
    blocks.push(
      mkSection(
        `:new: *${report.newFailures.length} New Failure(s)* (not seen in previous window)`,
      ),
    );
  }
