import { type ReportPollStatus, type ReportPollStep } from '@cnv-monitor/shared';
...
const STEP_PROGRESS: Record<ReportPollStep, number> = {
  'ai-mapping': 70,
  'ai-summary': 85,
  github: 10,
  gitlab: 25,
...
const createIdleState = (): ReportPollStatus => ({
  completedAt: null,
  currentStep: 'idle',
  error: null,
  logs: [],
...
const states = new Map<string, ReportPollStatus>();
...
export const getReportPollStatus = (component?: string): ReportPollStatus => {
  const state = states.get(resolveKey(component)) ?? createIdleState();
  return { ...state, logs: [...state.logs] };
};

export const getAllPollStatuses = (): Record<string, ReportPollStatus> => {
  const result: Record<string, ReportPollStatus> = {};
...
export const isReportPollRunning = (component?: string): boolean => {
...
export const startReportPoll = (component?: string): void => {
  const key = resolveKey(component);
...
export const stepReportPoll = (step: ReportPollStep, message: string, component?: string): void => {
  const key = resolveKey(component);
...
export const logReportPoll = (step: ReportPollStep, message: string, component?: string): void => {
  const key = resolveKey(component);
  const state = states.get(key);
  if (!state) return;
  state.logs.push({ message, step, timestamp: new Date().toISOString() });
...
export const completeReportPoll = (component?: string): void => {
  const key = resolveKey(component);
  const state = states.get(key);
  if (!state) return;
...
export const failReportPoll = (error: string, component?: string): void => {
  const key = resolveKey(component);
  const state = states.get(key);
  if (!state) return;
  state.status = 'failed';
